...
from core.config import ALLOWED_UPLOAD_EXTS
...
def load_file(
    ...
    if ext not in ALLOWED_UPLOAD_EXTS:
        raise ValueError(
            ...
            + ", ".join(f".{e}" for e in ALLOWED_UPLOAD_EXTS)
        )
    ...
    if ext == "xlsx":
        df = pd.read_excel(io.BytesIO(raw_bytes), engine="openpyxl", na_values=markers)
        encoding = "—"
    else:
    ...
    markers_found = []
    text_sample = raw_bytes[:200_000].decode(encoding if ext != "xlsx" else "latin-1", errors="ignore")
    for m in markers:
